...
#' str(efc.sub)
#'
#' @importFrom sjlabelled remove_all_labels set_labels get_labels get_label set_label
#' @export
copy_labels <- function(df_new, df_origin = NULL) {
  ...
  if (is.null(df_origin)) {
    ...
    # remove all labels
    df_new <- sjlabelled::remove_all_labels(df_new)
  } else {
    # check params
    if (is.data.frame(df_new) && is.data.frame(df_origin)) {
      ...
      df_new <- sjlabelled::set_label(df_new, label = sjlabelled::get_label(df_origin[, cn]))

      # same for value labels
      df_new <- suppressMessages(sjlabelled::set_labels(
        df_new,
